...
class Memorequest extends StatefulWidget {
  const Memorequest({super.key});

  @override
  State<Memorequest> createState() => _MemorequestState();
}

class _MemorequestState extends State<Memorequest> {
  String? selectedmemoTypeName;
  String? selectedmemoTypeId;
  ...
  void onUpload() async {
    ...
    if (result != null && result['status'] == true) {
      ...
      onmemorequest(); // your existing next step
    }
  }
  ...
  Widget getdetails() {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          AppUtils.buildNormalText(text: "MEMO TYPE"),
          const SizedBox(
            height: 5,
          ),
          DropdownSearch<String>(
            selectedItem: selectedmemoTypeName,
            items: const [
              "Warning",
              "Explanation",
              "Appreciation",
            ...
            onChanged: (String? value) {
              setState(() {
                selectedmemoTypeName = value;

                if (value == "Warning") {
                  selectedmemoTypeId = "1";
                } else if (value == "Explanation") {
                  selectedmemoTypeId = "2";
                } else if (value == "Appreciation") {
                  selectedmemoTypeId = "3";
                } else if (value == "Explanation") {
                  selectedmemoTypeId = "4";
                } else {
                  selectedmemoTypeId = null;
                }
              });
            },
          ...
          AppUtils.buildNormalText(text: "MEMO DESCRIPTION"),
          const SizedBox(
            height: 10,
          ),
          Container(
          ...
          CustomButton(
            onPressed: () {
              if (!validateMemoForm(context)) return;
              onmemorequest();
            },
            name: "Apply Memo Request",
            circularvalue: 30,
            fontSize: 16,
          )
        ],
  ...
  void onmemorequest() async {
    ...
    ApiService.AddMemo(json).then((response) {
      setState(() {
        loading = false;
      });
...
// Call this before posting the request
  bool validateMemoForm(BuildContext context) {
    if (selectedmemoTypeName == null || selectedmemoTypeName!.isEmpty) {
      AppUtils.errorsnackBar("Please select a Memo Type", context);
      return false;
    }

    if (subjectcontroller.text.trim().isEmpty) {
      AppUtils.errorsnackBar("Please enter Subject", context);
      return false;
    }

    if (descriptioncontroller.text.trim().isEmpty) {
      AppUtils.errorsnackBar("Please enter Memo Description", context);
      return false;
    }

    if (effectivedatecontroller.text.trim().isEmpty) {
      AppUtils.errorsnackBar("Please select Effective Date", context);
      return false;
    }

    // // Optional: Check attachment size if needed
    // if (attachcontroller.text.trim().isEmpty) {
    //   AppUtils.errorsnackBar("Please attach a file", context);
    //   return false;
    // }
